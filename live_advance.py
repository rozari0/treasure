...
class LiveAdvance:
    ...
    def on_new_com_data(self, *args, **kwargs):
        ...
        if data.get("action") == "disappear":
            print("Found Disappear command. Exiting.")
            sys.exit(1)
        
        if data.get('power') >= 0.5:
            post(f"http://192.168.0.87:8000/send_action?action={data.get("action")}")
...
def main():
    ...
    your_app_client_id = "Pihgb5Wx3nnXGwIeqJCgUUCYpe9fatO7Z1VUa48S"
    your_app_client_secret = "MsiYC2Gw7JCcm6f21GTzzbfPh5mFMwNa8Dacol1Lif4VlLyMlpeL9J1gTT8Auf4kQMoqHtFnXE8ePv3QbG28O1iMIEqsiUBqEmA167da3WKaUtrBeB1nftAcHeQVGFWn"

    # Init live advance
    l = LiveAdvance(your_app_client_id, your_app_client_secret)

    trained_profile_name = "bci"  # Please set a trained profile name here
